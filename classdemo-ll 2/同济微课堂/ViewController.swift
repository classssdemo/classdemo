...
//    @IBOutlet weak var tabBar: UITabBar!
    ...
    override func viewWillAppear(_ animated: Bool) {
        initSideMenu()
        UIApplication.shared.keyWindow?.rootViewController = AppDelegate.centerContainer
    }
    ...
    func initTabBar(){
        ...
        let myView = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "myview")
        myView.tabBarItem.title = nameArr[2]
        myView.tabBarItem.image = UIImage(named: picArr[2])
        myView.tabBarItem.setTitleTextAttributes([NSAttributedStringKey.font: UIFont(name: "Heiti SC", size: 11.0)!], for: .normal)
        
        
        self.selectedIndex = 0
        self.viewControllers = [homeView, listView, myView]
